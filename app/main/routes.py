...
def index():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(post)
        db.session.commit()
        flash(_('Сообщение успешно отправленно!'))
    ...
    return render_template('main.html', title='Home', form=form, posts=posts.items, next_url=next_url,
                           prev_url=prev_url)
...
def unfollow(username):
    ...
    if form.validate_on_submite():
        user = User.query.filter_by(username=username).first()
        if user is None:
            flash(_('Пользователь %(username)s не найден'))
            return redirect(url_for('main.index'))
        if user == current_user:
            flash(_('Вы не можете описаться от себя!'))
            return redirect(url_for('main.user', username=username))
        current_user.unfollow(user)
        db.session.commit()
        flash(_('Вы не подписанны на %(username)s!'))
    else:
        return redirect(url_for('main.user', username=username))
...
def explore():
    ...
    prev_url = url_for('main.explore', page=posts.prev_num) \
        if posts.has_prev else None
    return render_template('main.html', title=_('Explore'),
